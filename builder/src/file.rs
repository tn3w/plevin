    ...
    for part in written.parts {
        let held = match part {
            Part::Values(sheet) => {
                ...
                let blocks = numbers(&sheet.values, sheet.encoding == "signed");
                pack(
                    sheet.name,
                    blocks,
                    Vec::new(),
            ...
            Part::Index { name, keys, wide } => {
                ...
                pack(name, blocks, heads, width, count, 4096, 64, "index", "")
            }
            Part::Strings(pool) => {
                let count = pool.len();
...
fn coded(pool: &[String]) -> Vec<Vec<u8>> {
    pool.chunks(2048)
        .map(|chunk| {
            let groups: Vec<Vec<u8>> = chunk.chunks(32).map(group).collect();
...
fn addresses(keys: &[u128], wide: bool) -> (Vec<Vec<u8>>, Vec<u128>) {
    ...
    for chunk in keys.chunks(4096) {
        heads.push(chunk[0]);
        let groups: Vec<&[u128]> = chunk.chunks(64).collect();
...
fn pack(
    ...
    let book = match raw > TRAIN && blocks.len() > 8 {
        true => zstd::dict::from_samples(&blocks, BOOK).unwrap_or_default(),
        false => Vec::new(),
    };
